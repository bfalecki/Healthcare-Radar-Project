...
%% wydobycie jednowymiarowego przebiegu I/Q ### - RN zmiany
range_cell =5;
...
for i = 1:num_segments-1
    ...
    if gap_start <= gap_end
        ...
        signal_to_fill(gap_start:(gap_end+1)) = NaN;  % Complex NaN
    end
end

% Interpolacja
% ### - RN Tutaj ręcznie dobierałem MaxGap i sprawdzalem wyniki, przy
% innych dlugosciach nagran trzeba by bylo to dostosowac (juz pomiajajac
% usuniecie blędnych probek co myla interpolacje)
signal_interp = fillmissing(signal_to_fill, 'makima', 'MaxGap',max_gap*2);

% Gdyby jakies NaNy zostaly to wypelnij zerami
signal_interp(isnan(signal_interp)) = 0;

% ### - RN UWAGA tu po prostu podmieniam sygnaly bo nie chcialo mi
%%% sie edytowac reszty kodu
signal_filled = signal_interp;

% Plot 
real_interp = real(signal_interp);
imag_interp = imag(signal_interp);
figure(451)
plot(time_lags_filled, [real_interp.' imag_interp.'])
xlabel("Time [s]")
ylabel("Signal amplitude [-]")
legend("Real part", "Imag. part")
xlim(xlims)
title("Interpolated I/Q signal (using fillmissing)")
%% demodulacja fazy
phase = unwrap(angle(signal_filled));

% ### - RN Po interpolacji wydaje sie nie potrzebne - wprowadza błędy
